...
/* inverse s-box */
static const __u8 inv_sbox[256] = {
 ...
 * Xtime3[x] = x^Xtime2[x];
 */
static const __u8 x_time_9[256] = {
...
static const __u8 x_time_b[256] = {
...
static const __u8 x_time_d[256] = {
...
static const __u8 x_time_e[256] = {
 ...
 */
static void shift_rows(__u8 *state)
{
	__u8 tmp;
 ...
 */
static void inv_shift_rows(__u8 *state)
{
	__u8 tmp;
...
static void mix_sub_columns(__u8 *state)
{
	__u8 tmp[4 * AES_STATECOLS];
...
static void inv_mix_sub_columns(__u8 *state)
{
	__u8 tmp[4 * AES_STATECOLS];
	int  i;
 ...
 */
static void add_round_key(__u32 *state, __u32 *key)
{
	int idx;
...
{
	__u8 state[AES_STATECOLS * 4];
	__u32 round;

	memcpy(state, in, AES_STATECOLS * 4);
	add_round_key((__u32 *)state, (__u32 *)expkey);
	...
	for (round = 1; round < AES_ROUNDS + 1; round++) {
		...
		add_round_key((__u32 *)state,
			      (__u32 *)expkey + round * AES_STATECOLS);
	}
...
void aes_decrypt(__u8 *in, __u8 *expkey, __u8 *out)
{
	...
	add_round_key((__u32 *)state,
		      (__u32 *)expkey + AES_ROUNDS * AES_STATECOLS);
...
static void debug_print_vector(char *name, __u32 num_bytes, __u8 *data)
{
#ifdef DEBUG
	printf("%s [%d] @0x%08x", name, num_bytes, (__u32)data);
...
{
	__u8 zero_key[AES_KEY_LENGTH] = { 0 };
	__u8 tmp_data[AES_KEY_LENGTH];
	/* Convenient array of 0's for IV */
	__u8 *cbc_chain_data = zero_key;
	__u32 i;
...
{
	__u8 tmp_data[AES_KEY_LENGTH], tmp_block[AES_KEY_LENGTH];
	/* Convenient array of 0's for IV */
	__u8 cbc_chain_data[AES_KEY_LENGTH] = { 0 };
	__u32 i;
